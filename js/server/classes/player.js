// Use global Sql and LocalStorageDB (loaded via script tags)
// Create LocalStorageDB instance (replaces MySQL connection)
const conn = typeof LocalStorageDB !== 'undefined' ? new LocalStorageDB() : null;
...
class Player {
  ...
  allocateStat(stat) {
    ...
    if (this.canAffordStat(statName)) {
      ...
      this.stats[statName]++;
      this.save(false, true); // Save immediately when allocating stats
      return true;
    }
    return false;
  }
  ...
  allocateSkill(skillId) {
    ...
    this.save(false, true); // Save immediately when allocating skills
  }
    ...
    conn.query(query, function (error, result) {
      if (error) {
        ...
        if (result.length !== 0) {
          ...
          console.log("Player " + that.name + " loaded at (" + that.location.x + "," + that.location.y + ") on " + that.location.map);
          if (SOCKET_LIST[that.id]) {
            SOCKET_LIST[that.id].emit(
              "recievePlayer",
              Object.assign(that.exportObj(), { tester: result[0].tester || 0 })
            );
          }
        }
      }
    ...
    if (
      ...
      if (Math.floor(this.location.getDistance(target.location)) <= range) {
        GRAPHICS.push({
          ...
          targetType: "players",
          target: this.hash,
          value: getDirection(this.location, this.target.location),
        });
        if (this.queuedAttack && this.queuedAttack.type === "combat") {
          if (this.queuedAttack.attackType === "item") {
            ...
            if (this.skills[this.queuedAttack.id].consume()) {
              ...
              if (this.queuedAttack.type2 === "physical") {
                if (attemptAttack(this, this.target)) {
          ...
          if (attemptAttack(this, this.target)) {
            ...
            const crit =
              getRandomInt(0, 100) < this.calculatedStats.critRate;
            if (crit) {
              damage = Math.floor(
  ...
  calculateDamage() {
    ...
    if (percisionBonus > dmgMax) {
      ...
      return getRandomInt(dmgMin + percisionBonus, dmgMax);
    }
  }
  ...
  gainGold(gold) {
    if (gold > 0) {
      ...
      this.save(false, true); // Save immediately when gaining gold
    }
  }
  ...
  levelUp() {
    ...
    this.save(false, true); // Save immediately when leveling up
  }
  ...
  buyItem(id, qty = 1) {
    if (
      ...
      this.gold -= this.items[id].value * qty;
      this.save(false, true); // Save immediately when buying items
    }
  }
  ...
  sellItem(id, qty) {
    if (
      ...
      this.save(false, true); // Save immediately when selling items
    }
  }
  ...
  buyEquipment(id) {
    ...
    if (
      ...
      this.gold -= e.cost;
      e.removeTemplate();
      this.save(false, true); // Save immediately when buying equipment
    }
  }
    ...
    if (
      ...
      if (!returnValue) {
        NOTICES.push({
          ...
          type: "itemGain",
          image: "itemGainIconGold",
          string: numberWithCommas(Math.floor(i.cost / 11)) + " Gold ",
        });
      } else {
  ...
  moveToTarget() {
    ...
    if (this.target && !this.dead) {
      let path = calculatePath(this.location, this.target.location);
      let pathNew = [];
      if (path.length < 45 && path.length > 0) {
        const stop = trimPath(path, this.target.location, range);
  ...
  walk() {
    if (this.moves.length > 0) {
      ...
      // Save position periodically (every 10 moves to avoid excessive saves)
  ...
  equipCombatItem(id, slot) {
    if ([0, 1, 2, 3].includes(parseInt(slot))) {
      ...
      this.save(false, true); // Save immediately when equipping combat items
    }
  }
  ...
  equipCombatSkill(id, slot) {
    if ([0, 1, 2, 3].includes(parseInt(slot))) {
      ...
      this.save(false, true); // Save immediately when equipping combat skills
    }
  }
  ...
  equipItem(itemId) {
    ...
    this.save(false, true); // Save immediately when equipping items
  }
  ...
  unequipItem(itemId) {
    ...
    this.setEquipmentStats();
    this.save(false, true); // Save immediately when unequipping items
  }
