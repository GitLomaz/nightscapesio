class Item {
  ...
  consume() {
    if (this.quantity >= 1 && !this.disabled) {
      ...
      SOCKET_LIST[this.playerId].emit("disableItem", { id: that.id });
      return true;
    } else {
      return false;
    }
  ...
  sell(qty) {
    if (this.quantity >= qty && !this.disabled) {
      ...
      NOTICES.push({
        ...
        string:
          numberWithCommas(Math.floor(this.value / 11) * qty) +
          " Gold ",
      });
    }
  }
  ...
  use() {
    if (this.quantity >= 1 && !this.disabled) {
      const player = SOCKET_LIST[this.playerId].player;
      const that = this;
      let effect = 0;
      this.quantity--;
      ...
      switch (this.useType) {
        case 1:
          // Heal
          effect = Math.floor(
            getRandomInt(this.useValueMin, this.useValueMax) *
